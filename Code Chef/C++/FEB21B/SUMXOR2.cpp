...
int mod=998244353, ws[MaxN], inv[500005];
int book_keeping[500005], f[500005], gpa[1<<19], gpb[1<<19];
...
int nCr(int n, int r){
    if(r>n)return 0;
    return (((inv[r]*inv[n-r])%mod)*f[n])%mod;
}
...
void prepare(){
    ...
    f[1] = f[0] = 1;
    for(int i=2;i<200005;i++) book_keeping[i] = book_keeping[mod%i]*(mod-mod/i)%mod;

    for(int i=2;i<200005;i++) f[i]=(f[i-1]*i)%mod;

    for(int i=2;i<200005;i++) inv[i]=(book_keeping[i]*inv[i-1])%mod;
